...
var JFormValidator = new Class({
	initialize: function()
	{
		...
		this.setHandler('convencional',
			function (value) {
				
				regex=/^(0[4|2|3|5|6|7])(\d{7})$/;
				return regex.test(value);
			}
		);			
		this.setHandler('celular',
			function (value) {
				regex=/^(09)(\d{8})$/;
				return regex.test(value);
			}
	...
	validate: function(el)
	{
		...
		if ($(el).hasClass('required')) {
			var errcedula =  $('err' + el.id);
			if (!($(el).getValue())) {
				$(errcedula).innerHTML='Este campo es obligatorio.';
		...
		if ((handler) && (handler != 'none') && (this.handlers[handler]) && $(el).getValue()) {
			...
			if (this.handlers[handler].exec($(el).getValue()) != true) {
				switch(handler)
				{
					case 'convencional':
					{
						$(errcedulas).setHTML('El tel&eacute;fono debe incluir el prefijo de zona.');
						break;
					}
					case 'celular':
						$(errcedulas).setHTML('El tel&eacute;fono celular debe iniciar con 09.<br> Debe tener 10 d&iacute;gitos.');
						break;
					case 'numeric':
						$(errcedulas).setHTML('El campo s&oacute;lo admite n&uacute;meros.');
						break;
					case 'email':
						$(errcedulas).setHTML('Correo electr&oacute;nico inv&aacute;lido.');
						break;	
